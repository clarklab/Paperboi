...
bot = open("bot.js").read()
sl, bl = src.count("\n"), bot.count("\n")
mini = lambda s: "\n".join(l.strip() for l in s.splitlines() if l.strip())
html = (mini(src).replace("__WASM__", base64.b64encode(defl).decode())
            .replace("__BOT__", mini(bot))
            .replace("__CL__", str(cl)).replace("__SL__", str(sl)).replace("__BL__", str(bl))
